...
struct APIInterval {
    let expired = Calendar.current.date(byAdding: .minute, value: -(LoaderConfig.intervalInMinutes + 2), to: Date())!
    let valid = Calendar.current.date(byAdding: .minute, value: -(LoaderConfig.intervalInMinutes - 2), to: Date())!
}
